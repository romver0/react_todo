...
tasks.push({ _id: 11, title: "new task", status: "backlog" });
...
class Kanban extends React.Component {
  ...
  render() {
    ...
    return (
      <main>
        <section style={classes.board} className="jira">
          {labels.map((channel) => (
            <KanbanColumn status={channel}>
              <div style={classes.column} className='jira_column'>
                  ...
                  {tasks
                    ...
                    .map((item) => (
                      <KanbanItem id={item._id} onDrop={this.update}>
                        <div style={classes.item} className='jira_item'>{item.title}</div>
                      </KanbanItem>
                    ))}
                </div>
              </div>
